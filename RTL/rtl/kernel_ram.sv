...
module kernel_ram #(
  ...
  always_ff @(posedge clk) begin
    ...
    else if (wr_en && pWEIGHT_BASE_ADDR <= weight_addr && weight_addr < pWEIGHT_BASE_ADDR+pKERNEL_NUM*pULTRA_RAM_NUM) begin
  ...
  generate
    for (idx = 0; idx < pULTRA_RAM_NUM; idx = idx + 1) begin
      always_ff @(posedge clk) begin
        ...
        if (wr_en && idx == ram_idx && pWEIGHT_BASE_ADDR <= weight_addr && weight_addr < pWEIGHT_BASE_ADDR+pKERNEL_NUM*pULTRA_RAM_NUM)
          kernel_r[idx][wr_slot] <= weight_data;
        else
